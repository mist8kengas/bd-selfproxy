...
module.exports = class selfproxy_Main {
    /* main script */
    start(){
        BdApi.showToast("Your''e mom", {info: "success"})
        console.log("Works!", new Date());

        /* import packages && proxy list */
        const https = require("https"), http = require("http"), GlobalAgent = require("global-agent").bootstrap(), proxy = require("./proxy.json");

        /* main script */
        const init = new Promise((resolve, reject)=>{
            const timeout = 15000, timeoutReject = setTimeout(()=>{
                reject([408, `Timeout of ${timeout}ms reached`]);
            }, parseInt(timeout));

            /* randomizer */
            var ps_func = (()=>{
                let rand_region = Math.round(Math.random() * (Object.keys(proxy).length - 1)),
                    rand_country = Math.floor(Math.random() * (Object.keys(proxy)[rand_region].length - 1)),
                    region = Object.keys(proxy)[rand_region],
                    country = proxy[region][Object.keys(proxy[region])[rand_country]],
                    rand_addr = country.proxies[Math.round(Math.random() * (country.proxies.length - 1))],

                    ps_addr = rand_addr.split(":");

                return {
                    info: { region: region.split(":"), country: country.name },
                    addr: ps_addr[0],
                    port: ps_addr[1],
                    full: ps_addr.join(":")
                };
            ...
            /* set proxy config && start the tunnel */
            global.GLOBAL_AGENT.HTTP_PROXY = `http://${ps.full}`;

            /* test if it's working on HTTPS */
            const api_https = https.request("https://812125.xyz/tools/self-ip", (res)=>{
                let data = "";
                res.on("data", chunk => data += chunk);
                res.on("end", ()=>{
                    let output = {
                        "info": ps,
                        "response": ["HTTPS_GET", JSON.parse(data).output.ip]
                    }
        ...
        /* run main script with promise */
        init.then((data)=>{
            console.log("res", data);
            BdApi.showToast(`Connected to server in ${data.info.country}`, {info: "success"});
        }).catch(console.error);
        ...
        if(!savedSoundSetting) this.soundToggled = true;
        else if(savedSoundSetting === "true") this.savedSoundSetting = true;
        else if(savedSoundSetting === "false") this.savedSoundSetting = false;
        else{
            console.error(`Game Activity Toggle Error: soundToggle data somehow corrupted, not true/false: ${savedSoundSetting}`);
    ...
    createButton(){
        ...
        if(rows.length){
            ...
            for(let i = 0; i < rows.length; ++i){
                try{
                    if(
                        rows[i].firstElementChild.firstElementChild.firstElementChild.firstElementChild.getAttribute("d") === muteIconPath ||
                        rows[i].firstElementChild.firstElementChild.firstElementChild.firstElementChild.getAttribute("d") === micIconPath
                    ){
                        row = rows[i];
                        break;
                    }
